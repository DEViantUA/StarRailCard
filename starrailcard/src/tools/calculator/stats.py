...
_LINK_SCORE = "https://raw.githubusercontent.com/"+decrypt_url(n,42)+"/"+decrypt_url(r,35)+"/"+decrypt_url(k,1)+"/generate/weight.json"
_LINK_DATA = "https://raw.githubusercontent.com/"+decrypt_url(n,42)+"/"+decrypt_url(r,35)+"/"+decrypt_url(k,1)+"/generate/{name}.json"
...
def open_score(name):
    with open(_PATH/f"{name}.json", 'r') as file:
        data = json.load(file)
